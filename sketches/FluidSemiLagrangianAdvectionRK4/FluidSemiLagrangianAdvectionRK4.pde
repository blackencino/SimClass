...
void SemiLagrangianAdvectFirstOrder(int new_Q, int old_Q, int U, int V, float dt) {
  for (int j = 0; j < NY; ++j) {
    for (int i = 0; i < NX; ++i) {
      float h = -dt;
      
      // k1 step
      float cell_vel_u_k1 = State[U][IX(i,j)] / DXY;
      float cell_vel_v_k1 = State[V][IX(i,j)] / DXY;
     
      float fi = float(i) + h * cell_vel_u_k1;
      float fj = float(j) + h * cell_vel_v_k1;
...
// Semi-Lagrangian Advection of Q by U & V
void SemiLagrangianAdvectRK2(int new_Q, int old_Q, int U, int V, float dt) {
  for (int j = 0; j < NY; ++j) {
    for (int i = 0; i < NX; ++i) {
      float h = -dt;
      ...
      // k2 step
      float fi_k2 = float(i) + h*cell_vel_u_k1;
      float fj_k2 = float(j) + h*cell_vel_v_k1;
      float cell_vel_u_k2 = BiLinearResample(U, fi_k2, fj_k2) / DXY;
      float cell_vel_v_k2 = BiLinearResample(V, fi_k2, fj_k2) / DXY;
      
      float fi = float(i) + (h/2.0)*(cell_vel_u_k1 + cell_vel_u_k2);
      float fj = float(j) + (h/2.0)*(cell_vel_v_k1 + cell_vel_v_k2);
...
// Semi-Lagrangian Advection of Q by U & V
void SemiLagrangianAdvectRK4(int new_Q, int old_Q, int U, int V, float dt) {
  for (int j = 0; j < NY; ++j) {
    for (int i = 0; i < NX; ++i) {
      float h = -dt;
      ...
      // k2 step
      float fi_k2 = float(i) + (h/2.0)*cell_vel_u_k1;
      float fj_k2 = float(j) + (h/2.0)*cell_vel_v_k1;
      float cell_vel_u_k2 = BiLinearResample(U, fi_k2, fj_k2) / DXY;
      float cell_vel_v_k2 = BiLinearResample(V, fi_k2, fj_k2) / DXY;
      
      // k3 step
      float fi_k3 = float(i) + (h/2.0)*cell_vel_u_k2;
      float fj_k3 = float(j) + (h/2.0)*cell_vel_v_k2;
      float cell_vel_u_k3 = BiLinearResample(U, fi_k3, fj_k3) / DXY;
      float cell_vel_v_k3 = BiLinearResample(V, fi_k3, fj_k3) / DXY;
      
      // k4 step
      float fi_k4 = float(i) + h*cell_vel_u_k3;
      float fj_k4 = float(j) + h*cell_vel_v_k3;
      float cell_vel_u_k4 = BiLinearResample(U, fi_k4, fj_k4) / DXY;
      float cell_vel_v_k4 = BiLinearResample(V, fi_k4, fj_k4) / DXY;
      
      float fi = float(i) + (h/6.0)*(cell_vel_u_k1 +
        (cell_vel_u_k2 * 2.0) + (cell_vel_u_k3 * 2.0) + cell_vel_u_k4);
      float fj = float(j) + (h/6.0)*(cell_vel_v_k1 +
        (cell_vel_v_k2 * 2.0) + (cell_vel_v_k3 * 2.0) + cell_vel_v_k4);
...
void TimeStepFirstOrder(float dt) {
  SwapSoot();
  SwapVel();
  SemiLagrangianAdvectFirstOrder(StateSoot, StatePrevSoot, 
      StatePrevVelU, StatePrevVelV, dt);
  SemiLagrangianAdvectFirstOrder(StateVelU, StatePrevVelU, 
      StatePrevVelU, StatePrevVelV, dt);
  SemiLagrangianAdvectFirstOrder(StateVelV, StatePrevVelV, 
      StatePrevVelU, StatePrevVelV, dt);
}

void TimeStepRK2(float dt) {
  SwapSoot();
  SwapVel();
  SemiLagrangianAdvectRK2(StateSoot, StatePrevSoot, 
      StatePrevVelU, StatePrevVelV, dt);
  SemiLagrangianAdvectRK2(StateVelU, StatePrevVelU, 
      StatePrevVelU, StatePrevVelV, dt);
  SemiLagrangianAdvectRK2(StateVelV, StatePrevVelV, 
      StatePrevVelU, StatePrevVelV, dt);
}

void TimeStepRK4(float dt) {
  SwapSoot();
  SwapVel();
  SemiLagrangianAdvectRK4(StateSoot, StatePrevSoot, 
      StatePrevVelU, StatePrevVelV, dt);
  SemiLagrangianAdvectRK4(StateVelU, StatePrevVelU, 
      StatePrevVelU, StatePrevVelV, dt);
  SemiLagrangianAdvectRK4(StateVelV, StatePrevVelV, 
      StatePrevVelU, StatePrevVelV, dt);
}
...
void draw() {
  ...
  fill( 0.0 );
  if (TimeStepMethod == 0) {
    text("Semi-Lagrangian Advection : First Order", 10, 30);
  } else if (TimeStepMethod == 1) {
    text("Semi-Lagrangian Advection : RK2", 10, 30);
  } else {
    text("Semi-Lagrangian Advection : RK4", 10, 30);
  }
}
